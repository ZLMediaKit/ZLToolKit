...
int main() {
    ...
    //获取一个对象,该对象将被主线程持有，并且不会被后台线程获取并赋值  [AUTO-TRANSLATED:fa8654ed]
    ...
    thread_group group;
    //创建4个后台线程，该4个线程模拟循环池的使用场景，  [AUTO-TRANSLATED:a574c5f2]
    ...
    //等待3秒钟，此时循环池里面可用的对象基本上最少都被使用过一遍了  [AUTO-TRANSLATED:e5893625]
    // Wait for 3 seconds, at this time, the objects available in the loop pool have been used at least once
    sleep(3);

    //但是由于reservedObj早已被主线程持有，后台线程是获取不到该对象的  [AUTO-TRANSLATED:2c0e93ce]
    ...
    //显式释放对象,让对象重新进入循环列队，这时该对象应该会被后台线程持有并赋值  [AUTO-TRANSLATED:8d97a1af]
    ...
    //这时，reservedObj还在循环池内，引用应该还是有效的，但是值应该被覆盖了  [AUTO-TRANSLATED:1323f66e]
