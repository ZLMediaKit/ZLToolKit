...
public:
     ...
     * @return 是否成功
     */
    bool listen(uint16_t port, const string &local_ip = "0.0.0.0", int backlog = 1024);
     ...
     * @return 是否成功
     */
    bool bindUdpSock(uint16_t port, const string &local_ip = "0.0.0.0");
     ...
     * @param cb 回调
     */
    void setOnBeforeAccept(onBeforeAcceptCB cb);
     ...
     * @return 是否成功
     */
    bool setSendPeerAddr(const struct sockaddr *dst_addr, socklen_t addr_len = 0);
     ...
     * @param buffer
     */
    void setReadBuffer(const BufferRaw::Ptr &buffer);
