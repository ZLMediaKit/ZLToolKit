...
void Socket::connect_l(const string &url, uint16_t port, const onErrCB &con_cb_in, float timeout_sec, const string &local_ip, uint16_t local_port) {
    ...
    auto async_con_cb = std::make_shared<function<void(const SockNum::Ptr &)>>([weak_self, con_cb](const SockNum::Ptr &sock) {
        ...
        // Listen to whether the socket is writable, writable indicates that the connection to the server is successful
    ...
    if (isIP(url.data())) {
        ...
        WorkThreadPool::Instance().getExecutor()->async([url, port, local_ip, local_port, weak_task, poller]() {
            ...
            // Block DNS resolution is executed in the background thread
...
ssize_t Socket::onRead(const SockNum::Ptr &sock, const SocketRecvBuffer::Ptr &buffer) noexcept {
    ...
    while (_enable_recv) {
        ...
        try {
            ...
            // Catch exceptions here to prevent data from not being read thoroughly, and epoll edge trigger failure issues
            LOCK_GUARD(_mtx_event);
...
bool Socket::emitErr(const SockException &err) noexcept {
    ...
    _poller->async([weak_self, err]() {
        ...
        // Delay closing the socket, only remove its IO event, to prevent Session object destruction from failing to obtain fd-related information
...
ssize_t Socket::send(Buffer::Ptr buf, struct sockaddr *addr, socklen_t addr_len, bool try_flush) {
    if (!addr) {
        ...
        // This time the send did not specify the target address, but the target is specified through bindPeerAddr
...
int Socket::flushAll() {
    ...
    if (!_sock_fd) {
        ...
        // If the connection is already disconnected or the send timed out
        return -1;
    }
    if (_sendable) {
    ...
    if (_send_flush_ticker.elapsedTime() > _max_send_buffer_ms) {
        ...
        // If the oldest data in the send queue exceeds the timeout time limit, then disconnect the socket connection
    ...
    while (true) {
        if (event & EventPoller::Event_Read) {
            ...
            if (fd == -1) {
                ...
                // Maybe too many file descriptors are open: UV_EMFILE/UV_ENFILE
#if (defined(HAS_EPOLL) && !defined(_WIN32)) || defined(HAS_KQUEUE) 
                // 边缘触发，还需要手动再触发accept事件,    [AUTO-TRANSLATED:85fa9030]
                // Edge trigger, still need to manually trigger the accept event again
                ...
                _poller->doDelayTask(100, [weak_self, sock]() {
                    if (auto strong_self = weak_self.lock()) {
                        ...
                        // Process the accept event again after 100ms, maybe there is an idle fd
                ...
                // Temporarily do not process the accept event, wait 100ms and then manually trigger onAccept (only after EAGAIN reads empty can it be triggered again through epoll)
                return -1;
#else
                ...
                // Temporarily do not process the accept event, as it is a level trigger, it will automatically enter the onAccept function again next time
                return -1;
#endif
            }
            ...
            try {
                ...
                // Catch exceptions here to prevent socket from not accepting thoroughly, epoll edge trigger failure issues
                LOCK_GUARD(_mtx_event);
            ...
            if (!peer_sock) {
                ...
                // This is the default construction behavior, that is, the child Socket shares the parent Socket's poll thread and closes the mutex lock
            ...
            // Set up the fd, so that it can be accessed normally in the onAccept event
            ...
            shared_ptr<void> completed(nullptr, [peer_sock, sock](void *) {
                try {
                    ...
                    // Then add the fd to the poll monitoring (ensure that the onAccept event is triggered first and then the onRead event)
                    if (!peer_sock->attachEvent(sock)) {
                        // 加入poll监听失败，触发onErr事件，通知该Socket无效  [AUTO-TRANSLATED:e81fd478]
                        // Failed to add to poll monitoring, trigger the onErr event, and notify that the Socket is invalid
            ...
            try {
                // 此处捕获异常，目的是防止socket未accept尽，epoll边沿触发失效的问题  [AUTO-TRANSLATED:523d496d]
                // Catch the exception here to prevent the socket from not being accepted, and the epoll edge trigger is invalid
                LOCK_GUARD(_mtx_event);
                // 先触发onAccept事件，此时应该监听该Socket的onRead等事件  [AUTO-TRANSLATED:29734871]
                // First trigger the onAccept event, at this time, you should listen to the onRead event of the Socket
...
bool Socket::flushData(const SockNum::Ptr &sock, bool poller_thread) {
        ...
        do {
            {
                ...
                // The secondary send cache is empty, so we continue to consume the data in the primary cache
                LOCK_GUARD(_mtx_send_buf_waiting);
                if (!_send_buf_waiting.empty()) {
                    // 把一级缓中数数据放置到二级缓存中并清空  [AUTO-TRANSLATED:4884aa58]
                    // Put the data in the primary cache into the secondary cache and clear it
                    LOCK_GUARD(_mtx_event);
            ...
            // If the primary cache is also empty, it means that all data has been written to the socket
            if (poller_thread) {
                // poller线程触发该函数，那么该socket应该已经加入了可写事件的监听；  [AUTO-TRANSLATED:5a8e123d]
                // The poller thread triggers this function, so the socket should have been added to the monitoring of the writable event
                // 那么在数据列队清空的情况下，我们需要关闭监听以免触发无意义的事件回调  [AUTO-TRANSLATED:0fb35573]
                // So, in the case where the data queue is empty, we need to close the monitoring to avoid triggering meaningless event callbacks
    ...
    while (!send_buf_sending_tmp.empty()) {
        ...
        if (n > 0) {
            ...
            // All or part of the send is successful
            if (packet->empty()) {
                // 全部发送成功  [AUTO-TRANSLATED:38a7d0ac]
                // All send is successful
                send_buf_sending_tmp.pop_front();
                continue;
            }
            // 部分发送成功  [AUTO-TRANSLATED:bd6609dd]
            // Part of the send is successful
            if (!poller_thread) {
                // 如果该函数是poller线程触发的，那么该socket应该已经加入了可写事件的监听，所以我们不需要再次加入监听  [AUTO-TRANSLATED:917049f0]
                // If this function is triggered by the poller thread, the socket should have been added to the monitoring of the writable event, so we don't need to add monitoring again
        ...
        // None of the sends were successful
        int err = get_uv_error(true);
        if (err == UV_EAGAIN) {
        ...
        // Other error codes, exceptions occur
        if (sock->type() == SockNum::Sock_UDP) {
    ...
    if (!send_buf_sending_tmp.empty()) {
        ...
        // The secondary cache is not fully sent, indicating that the socket is not writable, return directly
        return true;
    }

    // 二级缓存已经全部发送完毕，说明该socket还可写，我们尝试继续写  [AUTO-TRANSLATED:2c2bc316]
    // The secondary cache has been fully sent, indicating that the socket is still writable, we try to continue writing
    // 如果是poller线程，我们尝试再次写一次(因为可能其他线程调用了send函数又有新数据了)  [AUTO-TRANSLATED:392684a8]
    // If it is the poller thread, we try to write again (because other threads may have called the send function and there is new data)
...
void Socket::enableRecv(bool enabled) {
    ...
    // When writable, do not listen for writable events
...
bool Socket::bindPeerAddr(const struct sockaddr *dst_addr, socklen_t addr_len, bool soft_bind) {
    ...
    if (soft_bind) {
        ...
        // Soft binding, only save the address
