...
namespace toolkit{
     ...
     */
        ...
        BASIC_STRING_VIEW_CONSTEXPR size_type find(basic_string_view<Char> v, size_type pos = 0) const noexcept{
        ...
        BASIC_STRING_VIEW_CONSTEXPR size_type find(Char ch, size_type pos = 0) const {
        ...
        BASIC_STRING_VIEW_CONSTEXPR size_type find(const Char* s, size_type pos, size_type count) const{
            return find(this_type(s, count), pos);
        }

        BASIC_STRING_VIEW_CONSTEXPR size_type find(const Char* s, size_type pos = 0) const{
            return find(this_type(s, strlen(s)), pos);
        }

        BASIC_STRING_VIEW_CONSTEXPR size_type find_first_of(const basic_string_view& v, size_type pos = 0) const noexcept{
            if(pos >= size_)
              return npos;
        ...
        BASIC_STRING_VIEW_CONSTEXPR size_type find_first_of(const Char* s, size_type pos, size_type count) const{
            return find_first_of(this_type(s, count), pos);
        }


        BASIC_STRING_VIEW_CONSTEXPR size_type find_first_of(const Char* s, size_type pos = 0) const{
        ...
        BASIC_STRING_VIEW_CONSTEXPR std::vector<toolkit::basic_string_view<Char>> split(const char* delim){
          std::vector<toolkit::basic_string_view<Char>> vec;
          size_type _begin = 0;
          size_type _end  = npos;
          while(_begin < size()  && (_end = this->find_first_of(delim, _begin)) != npos){
            vec.push_back(this->substr(_begin, _end - _begin));
            _begin = _end + 1;
          }
          if(_begin < size()){
            vec.push_back(this->substr(_begin));
          }
          return vec;
        }

        BASIC_STRING_VIEW_CONSTEXPR bool begin_with(const char* data){
